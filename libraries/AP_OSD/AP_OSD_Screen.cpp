...
void AP_OSD_Screen::draw_power(uint8_t x, uint8_t y)
{
    ...
    if (battery.power_watts(power_w)) {
        ...
        backend->write(x, y, false, fmt, osd->avg_power_w, SYMBOL(SYM_WATT));
    } else {
...
void AP_OSD_Screen::draw_roll_angle(uint8_t x, uint8_t y)
{
    ...
    char r;
    if (abs(roll) > level_symbol_max_angle) {
    ...
    if (one_decimal) {
        ...
        backend->write(x, y, false, format, r, abs(roll), SYMBOL(SYM_DEGR));
    } else {
        const char *format = roll < 9.95 ? "%c  %.0f%c" : (roll < 99.95 ? "%c %.0f%c" : "%c%.0f%c");
        backend->write(x, y, false, format, r, abs(roll), SYMBOL(SYM_DEGR));
    }
}
...
void AP_OSD_Screen::draw_pitch(uint8_t x, uint8_t y, float pitch)
{
    ...
    const float level_symbol_max_angle = one_decimal ? 0.049f : 0.45f;
    char p;
    if (abs(pitch) > level_symbol_max_angle) {
    ...
    if (one_decimal) {
        ...
        backend->write(x, y, false, format, p, abs(pitch), SYMBOL(SYM_DEGR));
    } else {
        const char *format = pitch < 9.5 ? "%c %.0f%c" : "%c%.0f%c";
        backend->write(x, y, false, format, p, abs(pitch), SYMBOL(SYM_DEGR));
    }
}
...
void AP_OSD_Screen::draw_aspd1(uint8_t x, uint8_t y)
{
    ...
    if (airspeed != nullptr && airspeed->healthy()) {
        const bool warning = (osd->warn_aspd_low > 0 && asp1 < osd->warn_aspd_low) || (osd->warn_aspd_high > 0 && asp1 > osd->warn_aspd_high);
...
void AP_OSD_Screen::draw_aspd2(uint8_t x, uint8_t y)
{
    ...
    if (airspeed != nullptr && airspeed->healthy(1)) {
        const bool warning = (osd->warn_aspd_low > 0 && asp2 < osd->warn_aspd_low) || (osd->warn_aspd_high > 0 && asp2 > osd->warn_aspd_high);
