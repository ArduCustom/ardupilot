...
const AP_Param::GroupInfo AP_TECS::var_info[] = {
    ...
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("THR_FF_FILT", 59, AP_TECS, _thr_ff_filter, 0.1f),
    ...
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("THR_FF_DAMP", 60, AP_TECS, _thr_ff_damp, 0.6f),
...
void AP_TECS::_update_height_demand(void)
{
    ...
    if (_auto_thr_gliding_state == ATGS_DISABLED) {
        ...
        if ((_hgt_dem - _hgt_dem_prev) > (_max_climb_rate * 0.1f)) {
            _hgt_dem = _hgt_dem_prev + _max_climb_rate * 0.1f;
        }
