...
const AP_Param::GroupInfo AP_PitchController::var_info[] = {
    ...
    // @Description: Pitch axis angle controller P gain.  Converts the difference between desired roll rate and actual roll rate into a motor speed output
    // @Range: 0.08 0.35
    // @Increment: 0.005
    // @User: Standard
    ...
    // @Description: Pitch axis angle controller I gain.  Corrects long-term difference in desired roll rate vs actual roll rate
    // @Range: 0.01 0.6
    // @Increment: 0.01
    // @User: Standard
    ...
    // @Description: Pitch axis angle controller I gain maximum.  Constrains the maximum motor output that the I gain will output
    // @Range: 0 1
    // @Increment: 0.01
    // @User: Standard
    ...
    // @Description: Pitch axis angle controller D gain.  Compensates for short-term change in desired roll rate vs actual roll rate
    // @Range: 0.001 0.03
    // @Increment: 0.001
    // @User: Standard
 ...
 */
// float AP_PitchController::_get_coordination_rate_offset(float aspeed, float eas2tas, float roll, float pitch, int32_t pitch_sensor, bool &inverted) const
...
float AP_PitchController::_get_servo_out(float desired_rate, float scaler, bool disable_integrator, bool ground_mode, const AP_PitchController::GSO_AHRS_Data &ahrs_data)
{
    ...
    bool inverted;
    // const float rate_offset = _get_coordination_rate_offset(aspeed, eas2tas, roll, pitch, pitch_sensor, inverted);
...
void AP_PitchController::convert_pid()
{
    ...
    if (angle_kP.configured_in_storage()) {
        return;
    }
