...
{
    if ((control_mode != &mode_rtl || !plane.rtl.loitering) && control_mode != &mode_loiter) {
        return false;
    }

    radius = plane.loiter.radius;
    return true;
}
...
void Plane::update_fbwb_speed_height(void)
{
    ...
    if (now - target_altitude.last_elev_check_us >= 100000) {
        ...
        int32_t alt_change_cm = rc_max_climb_rate() * elevator_input * dt * 100;
