...
const AP_Scheduler::Task Plane::scheduler_tasks[] = {
    ...
    SCHED_TASK(stabilize,             400,    200,  18),
    SCHED_TASK(set_servos,            400,    200,  21),
...
void Plane::update_control_mode(void)
{
    ...
    if (armed_tstamp_ms && now - armed_tstamp_ms > 1000) {
    ...
    if (disarmed_tstamp_ms && now - disarmed_tstamp_ms > 1000) {
