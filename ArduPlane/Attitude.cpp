...
float Plane::stabilize_roll_get_roll_out(float speed_scaler)
{
    ...
    return rollController.get_servo_out_using_angle_target(nav_roll_cd, speed_scaler, disable_integrator,
...
float Plane::stabilize_pitch_get_pitch_out(float speed_scaler)
{
    ...
    */
    ...
    return pitchController.get_servo_out_using_angle_target(demanded_pitch, speed_scaler, disable_integrator,
                                         ground_mode && !(plane.g2.flight_options & FlightOptions::DISABLE_GROUND_PID_SUPPRESSION));
}
...
void Plane::stabilize_acro(float speed_scaler)
{
    ...
    if (g.acro_locking && is_zero(roll_rate)) {
        ...
        SRV_Channels::set_output_scaled(SRV_Channel::k_aileron, rollController.get_servo_out_using_angle_error(roll_error_cd, nav_roll_cd, speed_scaler, true, false));
    } else {
        /*
    ...
    if (g.acro_locking && is_zero(pitch_rate)) {
        ...
        SRV_Channels::set_output_scaled(SRV_Channel::k_elevator, pitchController.get_servo_out_using_angle_target(nav_pitch_cd, speed_scaler, false, false));
    } else {
        /*
...
void Plane::calc_nav_yaw_ground(void)
{
    ...
    float angle = 0;
    const float gspd = v.length();
    bool reversing = reversed_throttle;
    if (gspd > 0.5f) {
        angle = ToDeg(wrap_2PI(atan2f(v.y, v.x) - yaw));
        reversing = angle >= 90 && angle <= 270;
    }

    if (reversing) {
        steering_control.steering = -steering_control.steering;
    }

    steering_control.steering = constrain_int16(steering_control.steering, -4500, 4500);
}

void Plane::set_auto_thr_gliding(bool gliding_requested)
{
    TECS_controller.set_auto_thr_gliding(gliding_requested);
    if (auto_thr_gliding_state == ATGS_GLIDING && !gliding_requested && !(ahrs.airspeed_sensor_enabled() || TECS_controller.is_using_synthetic_airspeed())) {
        auto_thr_gliding_state = ATGS_WAITING_FOR_CRUISE_THR;
    } else {
        auto_thr_gliding_state = gliding_requested ? ATGS_GLIDING : ATGS_DISABLED;
    }
}
...
void Plane::calc_nav_pitch()
{
    ...
    if (auto_thr_gliding_state != ATGS_GLIDING && control_mode->is_guided_mode() &&
